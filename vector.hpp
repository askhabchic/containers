...
namespace ft {
	...
	public:
		...
		// Construstors:	
		explicit vector (const allocator_type& allocator = allocator_type()) : _begin(NULL), _end(NULL), _cap_end(NULL), _alloc(allocator), _size(0) {
		...
		const_reverse_iterator 		rbegin() const	{ return const_reverse_iterator(iterator(_end)); };
		reverse_iterator 			rend()			{ return reverse_iterator(iterator(_begin - 1)); };
		const_reverse_iterator 		rend() const	{ return const_reverse_iterator(iterator(_begin - 1)); };
// return reverse_iterator(iterator(buffer + _size - 1)); };
		//Modifiers:
		template <class InputIterator>
		...
		insert (iterator position, InputIterator first, InputIterator last) {
			...
			size_type cap = capacity();
			if (!dif) return ;
			else if (dif > max_size() - size())
				throw std::length_error("vector<T> length error");
			else if (size() + dif > cap) {
				cap = capacity_alloc(dif);
				pointer ptr = _alloc.allocate(cap);
				pointer tmp_ptr;
				try	{
					tmp_ptr = std::uninitialized_copy(begin(), position, ptr);
					tmp_ptr = std::copy(first, last, tmp_ptr);
					std::uninitialized_copy(position, end(), tmp_ptr);
				}
				catch(const std::bad_alloc& ba) {
					destroy_dealloc(ptr, tmp_ptr, cap);
